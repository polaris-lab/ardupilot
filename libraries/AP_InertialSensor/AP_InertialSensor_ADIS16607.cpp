...
void AP_InertialSensor_ADIS16607::read_sensor_fifo(void)
{
    ...
    if (need_sync) {
        ...
        if (ch == 0) {
            ...
            if (!dev->transfer(buffer, ADIS16607_FIFO_SAMPLE_SIZE, buffer, ADIS16607_FIFO_SAMPLE_SIZE)) {
                return;
            }
    ...
    while (total_block) {
        ...
        if (!dev->transfer(buffer, (read_block * ADIS16607_FIFO_SAMPLE_SIZE + 2), buffer, (read_block * ADIS16607_FIFO_SAMPLE_SIZE + 2))) {
            need_sync = true;
            return;
        }
